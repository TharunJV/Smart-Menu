...
def init_db():
    ...
    with get_db() as conn:
        existing_admins = conn.execute("SELECT roll_no FROM user WHERE role='admin'").fetchall()
        existing_rolls  = {r["roll_no"] for r in existing_admins}
        default_admins  = [
            ("Admin One",  "admin1", "admin123"),
            ("Admin Two",  "admin2", "admin456"),
        ]
        for name, roll_no, pwd in default_admins:
            if roll_no not in existing_rolls:
                conn.execute(
                    "INSERT INTO user (name, roll_no, password, role) VALUES (?,?,?,?)",
                    (name, roll_no, generate_password_hash(pwd), "admin")
                )
...
def admin_admins():
    with get_db() as conn:
        admins = conn.execute(
            "SELECT id, name, roll_no, created_at FROM user WHERE role='admin' ORDER BY id"
        ).fetchall()
...
def create_admin():
    ...
    roll_no  = request.form.get("roll_no","").strip()
    password = request.form.get("password","").strip()
    if not all([name, roll_no, password]):
        flash("Please fill in all fields.", "error")
    ...
    try:
        with get_db() as conn:
            conn.execute(
                "INSERT INTO user (name, roll_no, password, role) VALUES (?,?,?,?)",
                (name, roll_no, generate_password_hash(password), "admin")
            )
        flash(f"✅ Admin '{name}' created successfully!", "success")
    except sqlite3.IntegrityError:
        flash("⚠️ That Roll No is already taken.", "error")
    return redirect(url_for("admin_admins"))
...
def delete_admin(uid):
    ...
    with get_db() as conn:
        admin_count = conn.execute(
            "SELECT COUNT(*) c FROM user WHERE role='admin'"
        ).fetchone()["c"]
        if admin_count <= 1:
            flash("⛔ Cannot delete the last admin account.", "error")
            return redirect(url_for("admin_admins"))
        conn.execute("DELETE FROM user WHERE id=? AND role='admin'", (uid,))
